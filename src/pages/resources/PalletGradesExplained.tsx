...
export default function PalletGradesExplained() {
  const relatedArticles = ARTICLES.filter(a => a.slug !== 'pallet-grades-explained').slice(0, 3);

  return (
    <>
      <Helmet>
        <title>Pallet Grades Explained: Grade A vs B vs C | Grunge Pallets</title>
        <meta name="description" content="Learn the differences between Grade A, Grade B, and Grade C pallets. Understand which pallet grade is right for your business needs and budget." />
      </Helmet>
      <MainLayout>
        <ArticleLayout
